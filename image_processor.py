...
def extract_character(image_path, character_name, output_dir, name, studentTaskId, promptType, min_contour_area=3500, aspect_ratio_threshold=0.3):
    # 이미지 다운로드
    ...
    if promptType == "character" or promptType == "CHARACTER":
        # 적응형 임계값 처리
        ...
        for contour in contours:
            ...
            # 흰색 레이어 생성 (이미지 크기와 동일)
            overlay = np.ones((height, width, 3), dtype='uint8') * 255

            # 흰색 레이어를 50% 투명도로 원본 이미지와 합성
            alpha = 0.5  # 투명도 값
            image_with_overlay = cv2.addWeighted(overlay, alpha, image, 1 - alpha, 0)


            # 이미지 저장
            output_path = os.path.join(output_dir, f"{character_name}_with_overlay.png")
            extracted_images = [output_path]
...
def process_and_upload_images(data):
    ...
    image_requests = data['imageRequest']  # imageRequest가 리스트임을 고려해야 함
    result = []
    
    for image_request in image_requests:
        name = image_request['name']
        imageUrl = image_request['imageUrl']
        promptType = image_request['promptType']
        ...
        result_image_arr = extract_character(imageUrl, name, output_dir, name, studentTaskId, promptType)
